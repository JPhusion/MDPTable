...
class widget_cal(widget):
    ...
    def draw(self, surface, frame_count):
        ...
        self.write_centre(self.position, surface, f"Upcoming Events", self.font_orkneyb, 25*scale, -150*scale)
        if len(self.events) < 1:
            self.write_centre(self.position, surface, f"No Events", self.font_orkneyb, 15*scale, 0*scale)
        counter = 0
        for event in self.events:
            if counter > 5:
                break
            self.write_centre(self.position, surface, f"{event[0]}", self.font_orkneyb, 15*scale, -100*scale + counter*120)
            self.write_centre(self.position, surface, f"{event[1]} at {event[2]}", self.font_orkneyb, 10*scale, -85*scale  + counter*120)
            counter += 1
